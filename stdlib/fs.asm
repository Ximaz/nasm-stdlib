...
; --------------------------------------------------------------------------------
; (__INT err: rax) close(__INT fd: dil);
;
; Dscription :
;
; Close a file using the fd (dil).
;
...
; --------------------------------------------------------------------------------
; (__SIZE_T written_bytes: rax) write(__INT fd: dil, __CHAR *buffer: rsi);
;
; Description :
;
; Write bytes from the buffer (rsi) into a fd (dil).
;
...
; --------------------------------------------------------------------------------
; (__INT read_bytes: rax) read(__CHAR *buffer: rsi, __INT fd: dil, __INT len: rdx);
;
; Description :
;
; Read n bytes (len: rdx) from the fd (dil) and store them into the buffer (rsi).
;
...
; --------------------------------------------------------------------------------
; (__OFF_T err: rax) lseek(__INT fd: dil, SEEK_<SET|CUR|END> context: dl, __OFF_T offset: rsi);
;
; Description :
;
; Set the cursor position of an fd (dil) according the context which can be :
